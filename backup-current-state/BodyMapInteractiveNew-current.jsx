...
const BodyMapInteractiveNew = ({ navigateTo }) => {
  ...
  const [regionalData, setRegionalData] = useState(null); // Add regional data state
  ...
  useEffect(() => {
    if (selectedMeridian) {
      // Convert meridian ID to correct filename
      const filenameMap = {
        'Lung': 'lung',
  ...
  // Load regional data when view changes
  useEffect(() => {
    const loadRegionalData = async () => {
      const fileMap = {
        'front': 'front_regions_cleaned_padded.json',
        'back': 'full_back_cleaned_padded.json', 
        'side': 'side_regions_cleaned_padded.json'
      };
      
      const filename = fileMap[currentView];
      if (filename) {
        try {
          const response = await fetch(`/improved_body_map_with_regions/Improved body models and regions/Improved body models and regions/${filename}`);
          const data = await response.json();
          console.log(`Loaded regional data for ${currentView}:`, data);
          setRegionalData(data);
        } catch (error) {
          console.error(`Failed to load regional data for ${currentView}:`, error);
            ...
            <div className="flex flex-col lg:flex-row gap-6 items-start">
              ...
              <div className="flex-1">
                <div className="relative bg-gray-800 rounded-lg overflow-hidden">
                  ...
                  <div
                    className="absolute w-1 h-1 bg-red-500 rounded-full border-2 border-white shadow-lg z-10"
                    style={{
                      left: "50%",
                      top: "50%",
                ...
                <div className="relative bg-gray-800 rounded-lg overflow-hidden">
                  ...
                  {points.map((point, index) => (
                    <button
                      ...
                      className="point-button absolute bg-red-500 rounded-full border-2 border-white hover:bg-red-400 hover:scale-125 transition-all shadow-lg cursor-pointer w-1 h-1 sm:w-1.5 sm:h-1.5"
                      style={{
                        left: `${point.x * 100}%`,
                        top: `${point.y * 100}%`,
                        transform: "translate(-50%, -50%)"
