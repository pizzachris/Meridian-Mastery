  ...
  useEffect(() => {
    const loadSvgAndPoints = async () => {
      try {        // Load appropriate SVG
        ...
        const filteredPoints = flashcardsData.flashcards.filter(point => 
  ...
  const getZoomedPointCoordinates = (point, regionConfig) => {
    ...
    const coordinateMap = {      'head-neck': {
        ...
        front: {
          ...
          // Lung Meridian points in chest area
          'LU1': { x: 125, y: 150 }, // Central Residence - 1st intercostal space
          'LU1_R': { x: 175, y: 150 }, // Right side
          'LU2': { x: 122, y: 145 }, // Cloud's Door - below clavicle
          'LU2_R': { x: 178, y: 145 }, // Right side
          'ST25': { x: 120, y: 210 }, // Side abdomen
          'ST25_R': { x: 180, y: 210 } // Side abdomen
        },
        back: {
          'GV14': { x: 150, y: 115 },
          'GV11': { x: 150, y: 170 },
        ...
        front: {
          // Lung Meridian (LU) - Complete mapping for zoomed arm view
          'LU1': { x: 125, y: 145 }, // Central Residence - chest area
          'LU1_R': { x: 175, y: 145 }, // Right side
          'LU2': { x: 118, y: 140 }, // Cloud's Door - below clavicle
          'LU2_R': { x: 182, y: 140 }, // Right side
          'LU3': { x: 110, y: 160 }, // Heaven's Residence - upper arm
          'LU3_R': { x: 190, y: 160 }, // Right side
          'LU4': { x: 108, y: 170 }, // Gallantry - upper arm
          'LU4_R': { x: 192, y: 170 }, // Right side
          'LU5': { x: 95, y: 190 }, // Elbow crease radial
          'LU5_R': { x: 205, y: 190 }, // Right side
          'LU6': { x: 88, y: 210 }, // Forearm radial
          'LU6_R': { x: 212, y: 210 }, // Right side
          'LU7': { x: 82, y: 235 }, // Wrist radial
          'LU7_R': { x: 218, y: 235 }, // Right side
          'LU8': { x: 80, y: 245 }, // Wrist point
          'LU8_R': { x: 220, y: 245 }, // Right side
          'LU9': { x: 78, y: 255 }, // Wrist crease radial
          'LU9_R': { x: 222, y: 255 }, // Right side
          'LU10': { x: 75, y: 270 }, // Thenar eminence
          'LU10_R': { x: 225, y: 270 }, // Right side
          'LU11': { x: 72, y: 285 }, // Thumb nail point
          'LU11_R': { x: 228, y: 285 }, // Right side
          
          // Large Intestine and other arm points
      ...
      'feet': {
        ...
        back: {
          ...
          'BL62': { x: 120, y: 565 },
          'BL62': { x: 180, y: 565 },
          'KI1': { x: 125, y: 585 },          'KI1': { x: 175, y: 585 }
        }
      }
    }
    ...
    // Try to find point by code or name, including bilateral variants
    let coords = regionMap[viewSide][point.code] || 
                regionMap[viewSide][point.name] ||
                regionMap[viewSide][point.english_name]

    // If not found, try with _R suffix for right side bilateral points
    if (!coords && point.bilateral === "Yes") {
      coords = regionMap[viewSide][point.code + '_R'] || 
               regionMap[viewSide][point.name + '_R'] ||
               regionMap[viewSide][point.english_name + '_R']
    }

    // If still not found and this is a lung meridian point, try the standard LU format
    if (!coords && point.point_number && point.point_number.startsWith('LU')) {
      coords = regionMap[viewSide][point.point_number] ||
               regionMap[viewSide][point.point_number + '_R']
    }
